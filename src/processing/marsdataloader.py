...
# Assumption: dataset to be loaded is the same as the dataset specified to C.DATA_SUBDIR
...
class MARSDataLoader():
    ...
    def __init__(self,
        ...
        self.is_test_only_dataset = C.IS_TEST_ONLY_DATASET
            ...
            try:
                ...
                if missing_feats.intersection(C.INIT_FEATURES):
                    ...
                    generate_feature_files(self.window,
                                           ...
                                           self.data_dir,
                                           show_pbar=show_pbar,
                                           test_only_dataset=self.is_test_only_dataset)
        ...
        if self.is_test_only_dataset:
            self.total_sample_size = 0
            self.crash_sample_size = 0
            self.noncrash_sample_size = 0
        else:
    ...
    def retrieve_inds(self, get_train_split: bool):
        ...
        if get_train_split:
            if self.is_test_only_dataset:
                raise ValueError(f"This dataset at {self.data_dir} is test-only! You cannot use "
                                 f"the dataset for training purposes.")
